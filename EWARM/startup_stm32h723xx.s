...
;;
	THUMB
	PUBWEAK ExitRun0ModeToBypassMode
	SECTION .text:CODE:NOROOT:REORDER(1)
ExitRun0ModeToBypassMode
	MOV 	R1, #0x4800	; base address of PWR region is 0x58024800. writes 0x4800 into lower 16 bits of R1
	MOVT 	R1, #0x5802	; writes 0x5802 into upper 16 bits of R1
	LDR 	R0,[R1, #+12]	; loads R0 from the address in R1 + 12 bytes
	BIC 	R0,R0,#0x2	; disable LDO
	ORR	R0,R0,#0x1	; enable BYPASS
	STR 	R0,[R1, #+12]   ; stores R0 into the address in R1 + 12 bytes
wait_actvosrdy:
	LDR 	R2,[R1, #+4]
	LSLS 	R0,R2,#+18
...
Reset_Handler
	LDR 	R0, =ExitRun0ModeToBypassMode
	BLX 	R0
        LDR     R0, =SystemInit
        BLX     R0
